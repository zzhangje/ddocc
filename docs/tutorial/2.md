...
The left and right joysticks control the forward movement of the robot's left and right wheels, respectively. The top-left trigger is used for the intake: press to extend the coral, and release to retract it. The top-right trigger is used to adjust the angle of intake.
