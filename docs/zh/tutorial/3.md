...
public class ChassisTeleop extends Command {
  public ChassisTeleop(
    ...
    withInterruptBehavior(InterruptionBehavior.kCancelSelf); // 关键：声明命令冲突时的处理方式
  }
}
```
`addRequirements()`方法告诉命令调度器这个命令需要独占访问`chassis`子系统。当该命令运行时，如果其他命令同样需要访问这个子系统，`withInterruptBehavior()`方法告诉命令调度器，是中断自己还是拒绝其他命令运行。
