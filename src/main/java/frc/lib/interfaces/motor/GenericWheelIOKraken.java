...
public class GenericWheelIOKraken implements GenericWheelIO {
  ...
  @Getter private final StatusSignal<Angle> position;
  private final StatusSignal<Voltage> outputVoltage;
  private final StatusSignal<Current> supplyCurrent;
  private final StatusSignal<Current> statorCurrent;

  // Control request
  private final VelocityTorqueCurrentFOC velocitySetter = new VelocityTorqueCurrentFOC(0.0);
  ...
  public GenericWheelIOKraken(String name, CanId id, TalonFXConfiguration config) {
    this.name = name;
    var wrappedName = "[" + name + "]";
    master = new TalonFX(id.id(), id.bus());

    Phoenix6Helper.checkErrorAndRetry(
        wrappedName + " zero wheel position", () -> master.setPosition(0.0));
    Phoenix6Helper.checkErrorAndRetry(
        wrappedName + " clear sticky fault", master::clearStickyFaults);
    Phoenix6Helper.checkErrorAndRetry(
        wrappedName + " config", () -> master.getConfigurator().apply(config));
    this.config = config;
    ...
    Phoenix6Helper.checkErrorAndRetry(
        ...
        () ->
            BaseStatusSignal.setUpdateFrequencyForAll(
                ...
                statorCurrent));
    Phoenix6Helper.checkErrorAndRetry(
        wrappedName + " optimizate CAN utilization", master::optimizeBusUtilization);
  }

  public GenericWheelIOKraken withFollower(CanId id, boolean invert) {
    var slave = new TalonFX(id.id(), id.bus());

    var slaveConfig = new TalonFXConfiguration();
    slaveConfig.MotorOutput.NeutralMode = config.MotorOutput.NeutralMode;

    var slaveSetter = new Follower(master.getDeviceID(), invert);
