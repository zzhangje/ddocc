...
public class GenericRollerIOSimFlywheel implements GenericRollerIO {
  private final FlywheelSim sim;
  private final ProfiledPIDController pid =
      new ProfiledPIDController(
          0.0, 0.0, 0.0, GenericRollerIOSim.UNLIMITED, Constants.LOOP_PERIOD_SEC);
  private final SlewRateLimiter voltageLimiter = new SlewRateLimiter(2.5);

  private double appliedVoltageVolt = 0.0;

  public GenericRollerIOSimFlywheel(DCMotor motor, double reduction, double moiJKgMeter2) {
    sim =
        new FlywheelSim(LinearSystemId.createFlywheelSystem(motor, moiJKgMeter2, reduction), motor);
  }
  ...
  public void setVel(double velRadPerSec) {
    pid.setConstraints(GenericRollerIOSim.UNLIMITED);
